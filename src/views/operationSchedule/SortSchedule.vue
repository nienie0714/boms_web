<template>
    ...
    <div class="templateContent">
        <div class="query-top">
            <query-row :data="queryParam" @handleEnter="queryDataReq(2)" @change="dateChange"></query-row>
            <div class="toolbar">
				...
				<el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <div v-if="item.key == 'empName'">
                            <span :class="scope.row[item.key]?'bold-underline':''" @click="openDetail({type: 'detail', row: scope.row})" :title="scope.row[item.key]">{{scope.row[item.key]}}</span>
                        </div>
                        <div v-if="item.key == 'isWork'">
                            <!-- 1-上班  2-休息 -->
                            <span :class="['operate-icon',scope.row.isWork == 1?'active':'']">{{scope.row.isWork == 1?'上班':'休息'}}</span>
                        </div>
					</template>
				</el-table-column>
                <el-table-column
					...
					:min-width="item.minWidth">
                    <template slot-scope="scope" >
                        <span :title="scope.row[item.key]">{{showValue(scope.row, item)}}</span>
                    </template>
				</el-table-column>
			</template>  
            <el-table-column 
				...
				<template slot-scope="scope">
                    <div class="operation-cell">
                        <div v-if="permissions.update && scope.row.dutyDate >= todayStr && queryData.dutyDate >= todayStr" class="table-opr update" @click="openDetail({type: 'update', row: scope.row})"></div>
            ...
            <div class="right">
                ...
                <div class="toolbar" >
                    <div class="tool-button export" @click="openExport">
                        <div class="icon"></div>
                        <div class="label" style="font-weight:bolder">导出</div>
                    </div>
                    <div v-if="permissions.insert && queryData.dutyDate >= todayStr" class="tool-button insert" @click="openDetail({type: 'insert'})">
                        <div class="icon"></div>
                        <div class="label" style="font-weight:bolder">新增</div>
                    </div>
                </div>
            </div>
        </div>
        </div>
        <detail :visible="detail.visible" :data="detail.data" :type="detail.type" :date="queryData.dutyDate" @handleSubmit="handleSubmit" @handleClose="handleClose"></detail>
...
import Detail from './detail/SortDetail'
...
export default {
    ...
    data() {
        return {
            ...
            insertUrl: 'sortSchedule',
            updateUrl: 'sortSchedule',
            removeUrl: 'sortSchedule',
            queryParam: [
                {
                    key: 'dutyDate',
                    label: '值班日期',
            ...
            tableData: {
                ...
                column: [
                    ...
                    {key: 'chute',  label: '保障滑槽号', minWidth: 300,},
                    {key: 'flightNo',  label: '保障航班号', minWidth: 300,},
                    {key: 'isWork',  label: '状态', minWidth: 100,type: 'slot'},
                ],
                data: []
            },
            visibleEnable: false,
    ...
    methods: {
        ...
        dateChange(val,item) {
            if(item.type == 'elDate') {
                this.$set(this.queryData, 'dutyDate', val,item)
                if(!val)  {
                    this.$msg.error({
                        info: ' 值班日期不能为空!',
                    })
